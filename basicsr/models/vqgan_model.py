...
@MODEL_REGISTRY.register()
class VideoGANModel(VQGANModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.optimizer_g.zero_grad()
        self.output, (l_codebook, l_motion_codebook), quant_stats = self.net_g(self.gt)

        l_codebook = l_codebook*self.l_weight_codebook
        l_motion_codebook = l_motion_codebook*self.l_weight_codebook
        ...
        if current_iter % self.net_d_iters == 0 and current_iter > self.net_g_start_iter:
            ...
            if current_iter > self.net_d_start_iter:
                ...
                if hasattr(self.net_g, 'module'):
                    last_layer = self.net_g.module.generator.temporal_blocks[-1].time_mixer.mix_factor
                else:
                    last_layer = self.net_g.generator.temporal_blocks[-1].time_mixer.mix_factor
                d_weight = self.calculate_adaptive_weight(recon_loss, l_g_gan, last_layer, disc_weight_max=1.0)
                d_weight *= self.adopt_weight(1, current_iter, self.net_d_start_iter)
                d_weight *= self.disc_weight # tamming setting 0.8
                l_g_total += d_weight * l_g_gan
            ...
            l_g_total += l_codebook
            loss_dict['l_codebook'] = l_codebook
            l_g_total += l_motion_codebook
            loss_dict['l_motion_codebook'] = l_motion_codebook
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if save_img:
                ...
                # imwrite(sr_img, save_img_path)
                vidwriter = VideoWriter(save_img_path, sr_img.shape[1], sr_img.shape[2], 8, None)
...
@MODEL_REGISTRY.register()
class VideoGANModelv0(VQGANModel):
...
@MODEL_REGISTRY.register()
class VideoGANModelV2(VQGANModel):
...
@MODEL_REGISTRY.register() 
class VideoGANModelV2Motion(VideoGANModelV2):
    def optimize_parameters(self, current_iter):
        logger = get_root_logger()
        loss_dict = OrderedDict()
        if self.opt['network_g']['quantizer'] == 'gumbel':
