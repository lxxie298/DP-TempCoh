...
@MODEL_REGISTRY.register()
class CodeFormerFinetuneGenerator(SRModel):
    def feed_data(self, data):
        self.gt = data['gt'].to(self.device)
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output, logits, lq_feat = self.net_g(self.input, w=0, detach_16=True)
        ...
        if current_iter % self.net_d_iters == 0 and current_iter > self.net_g_start_iter:
            ...
            if not 'transformer' in self.opt['network_g']['fix_modules']:
                ...
                if self.cross_entropy_loss:
                    # b(hw)n -> bn(hw)
                    cross_entropy_loss = F.cross_entropy(logits.permute(0, 2, 1),
                                                         self.idx_gt) * self.entropy_loss_weight
                    l_g_total += cross_entropy_loss
                    loss_dict['cross_entropy_loss'] = cross_entropy_loss
            ...
            # perceptual loss
            output_bf = rearrange(self.output, 'b f c h w -> (b f) c h w')
            gt_bf = rearrange(self.gt, 'b f c h w -> (b f) c h w')
            if self.cri_perceptual:
                l_g_percep = self.cri_perceptual(output_bf, gt_bf)
                l_g_total += l_g_percep
                loss_dict['l_g_percep'] = l_g_percep

            # gan loss
            if current_iter > self.net_d_start_iter:
                fake_g_pred = self.net_d(output_bf)
                l_g_gan = self.cri_gan(fake_g_pred, True, is_disc=False)
                recon_loss = l_g_pix + l_g_percep
                if not self.fix_generator:
        ...
        if current_iter > self.net_d_start_iter:
            ...
            gt_bf = rearrange(self.gt, 'b f c h w -> (b f) c h w')
            # real
            real_d_pred = self.net_d(gt_bf)
            l_d_real = self.cri_gan(real_d_pred, True, is_disc=True)
            loss_dict['l_d_real'] = l_d_real
            loss_dict['out_d_real'] = torch.mean(real_d_pred.detach())
            l_d_real.backward()
            # fake
            fake_d_pred = self.net_d(output_bf.detach())
            l_d_fake = self.cri_gan(fake_d_pred, False, is_disc=True)
            loss_dict['l_d_fake'] = l_d_fake
            loss_dict['out_d_fake'] = torch.mean(fake_d_pred.detach())
            l_d_fake.backward()
...
@MODEL_REGISTRY.register()
class VideoCodeFormerJointModel(CodeFormerJointModel):
    ...
    def test(self):
        with torch.no_grad():
            if hasattr(self, 'net_g_ema'):
                self.net_g_ema.eval()
                self.output_w1, _, _ = self.net_g_ema(self.input, w=1)
                self.output_w0, _, _ = self.net_g_ema(self.input, w=0)
                self.output_w05, _, _ = self.net_g_ema(self.input, w=0.5)
            else:
                logger = get_root_logger()
                logger.warning('Do not have self.net_g_ema, use self.net_g.')
                self.net_g.eval()
    ...
    def get_current_visuals(self):
        out_dict = OrderedDict()
        out_dict['gt'] = self.gt.detach().cpu()
        out_dict['result_w0'] = self.output_w0.detach().cpu()
        out_dict['result_w1'] = self.output_w1.detach().cpu()
        out_dict['result_w05'] = self.output_w05.detach().cpu()
        return out_dict
    ...
    def feed_data(self, data):
        ...
        self.input_large_de = data['lq_large_de'].to(self.device)
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.generate_idx_gt:
            ...
            self.idx_gt = min_encoding_indices.view(self.b, -1)  # b(fhw)
        ...
        if current_iter % self.net_d_iters == 0 and current_iter > self.net_g_start_iter:
            ...
            # pixel loss 
            if not large_de: # when large degradation don't need image-level loss
                output = rearrange(self.output, 'b f c h w -> (b f) c h w')
                gt = rearrange(self.gt, 'b f c h w -> (b f) c h w')
                if self.cri_pix:
                ...
                # perceptual loss
                if self.cri_perceptual:
                    l_g_percep = self.cri_perceptual(output, gt)
                    l_g_total += l_g_percep
                    loss_dict['l_g_percep'] = l_g_percep
                ...
                if  current_iter > self.net_d_start_iter:
                    ...
                    if not self.fix_generator:
                        ...
                        largest_fuse_size = self.opt['network_g']['connect_list'][-1]
                        # last_layer = self.net_g.module.fuse_convs_dict[largest_fuse_size].shift[-1].weight
                        last_layer = self.net_g.module.fuse_convs_dict[largest_fuse_size].zero_conv.weight
                        d_weight = self.calculate_adaptive_weight(recon_loss, l_g_gan, last_layer, disc_weight_max=1.0)
        ...
        # optimize net_d
        if not large_de:
            output = rearrange(self.output, 'b f c h w -> (b f) c h w', f=frame_size)
            gt = rearrange(self.gt, 'b f c h w -> (b f) c h w', f=frame_size)
            if current_iter > self.net_d_start_iter:
                for p in self.net_d.parameters():
                    p.requires_grad = True
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            visuals = self.get_current_visuals()
            # sr_img = tensor2img([visuals['result']])
            sr_img_w1 = visuals['result_w1']
            sr_img_w1 = self.tensor2numpy(sr_img_w1[0])
            sr_img_w0 = visuals['result_w0']
            sr_img_w0 = self.tensor2numpy(sr_img_w0[0])
            sr_img_w05 = visuals['result_w05']
            sr_img_w05 = self.tensor2numpy(sr_img_w05[0])

            input_img = self.tensor2numpy(self.input[0])


            if 'gt' in visuals:
                gt_img = self.tensor2numpy(visuals['gt'][0])
                del self.gt
            ...
            if save_img:
                ...
                if 'gt' in visuals:
                    out_img = np.concatenate([input_img, sr_img_w1,sr_img_w05,sr_img_w0, gt_img],axis=2)
                else:
                    out_img = np.concatenate([input_img, sr_img_w1,sr_img_w05,sr_img_w0],axis=2)
