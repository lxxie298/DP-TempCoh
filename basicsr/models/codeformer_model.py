    ...
    def optimize_parameters(self, current_iter):
        ...
        if current_iter % self.net_d_iters == 0 and current_iter > self.net_g_start_iter:
            ...
            if self.hq_feat_loss: # codebook loss 
                ...
                l_g_total = l_g_total + l_feat_encoder
            ...
            if self.cross_entropy_loss:
                ...
                l_g_total = l_g_total + cross_entropy_loss
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if save_img:
                ...
                sr_img = np.concatenate([lq_img, sr_img, gt_img],axis=0)
...
class InpaintingModel(SRModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.hq_feat_loss:
            # quant_feats
            quant_feat_gt = self.net_g.module.quantize.get_codebook_feat(self.idx_gt, shape=[batch_size*frame_size,16,16,256])
        ...
        if current_iter % self.net_d_iters == 0 and current_iter > self.net_g_start_iter:
            ...
            if self.fidelity_weight > 0: # when fidelity_weight == 0 don't need image-level loss
                # pixel loss
                if self.cri_pix:
                    l_g_pix = self.cri_pix(output_bf, gt_bf)
                    l_g_total += l_g_pix
                    loss_dict['l_g_pix'] = l_g_pix
            ...
            l_g_total = l_g_total + self.net_g.module.position_emb.sum()*0.0
            l_g_total.backward()
            self.optimizer_g.step()
