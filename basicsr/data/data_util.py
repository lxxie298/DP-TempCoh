...
def brush_stroke_mask_for_vid(video, color=(255,255,255)):
    min_num_vertex = 8
    max_num_vertex = 28
    mean_angle = 2*math.pi / 5
    angle_range = 2*math.pi / 12
    ...
    # min_width = 80
    # max_width = 120
    def generate_mask(H, W, video=None):
        average_radius = math.sqrt(H*H+W*W) / 8
        mask = Image.new('RGB', (W, H), 0)
        if video is not None: mask = video # Image.fromarray(img)
    ...
    mask = generate_mask(height, width, video)
    return mask