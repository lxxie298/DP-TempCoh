...
from basicsr.data.ffhq_blind_dataset import align_images_by_width, resize_random_scale_and_random_crop_video
import os, glob
...
@DATASET_REGISTRY.register()
class HQVideoJointDatasetImgSeqV2(data.Dataset):

    def __init__(self, opt):
        super(HQVideoJointDatasetImgSeqV2, self).__init__()
        logger = get_root_logger()
        self.opt = opt
        # file client (io backend)
        self.file_client = None
    ...
    def __getitem__(self, index):
        ...
        # generate in_large with large degradation
        frames_in_large = deepcopy(frames_gt)
        ...
        if self.use_corrupt:
            ...
            if self.use_motion_kernel and random.random() < self.motion_kernel_prob:
                ...
                k = self.motion_kernels[f'{m_i:02d}']
                # img_in_large = cv2.filter2D(img_in_large,-1,k)
                frames_in_large = [cv2.filter2D(f,-1,k) for f in frames_in_large]
            ...
            kernel = gaussian_kernels.random_mixed_kernels(
                ...
                noise_range=None)
            # img_in_large = cv2.filter2D(img_in_large, -1, kernel)
            frames_in_large = [cv2.filter2D(f, -1, kernel) for f in frames_in_large]

            # downsample
            scale = np.random.uniform(self.downsample_range_large[0], self.downsample_range_large[1])
            # img_in_large = cv2.resize(img_in_large, (int(self.gt_size // scale), int(self.gt_size // scale)), interpolation=cv2.INTER_LINEAR)
            frames_in_large = [cv2.resize(f, (int(self.gt_size // scale), int(self.gt_size // scale)), interpolation=cv2.INTER_LINEAR) for f in frames_in_large]

            # noise
            if self.noise_range_large is not None:
                noise_sigma = np.random.uniform(self.noise_range_large[0] / 255., self.noise_range_large[1] / 255.)
                frames_in_large_ = []
                for f in frames_in_large:
                    noise = np.float32(np.random.randn(*(f.shape))) * noise_sigma
                    f = f + noise
                    f = np.clip(f, 0, 1)
                    frames_in_large_.append(f)
                frames_in_large = frames_in_large_
            ...
            # resize to in_size
            # img_in_large = cv2.resize(img_in_large, (self.in_size, self.in_size), interpolation=cv2.INTER_LINEAR)
            frames_in_large = [cv2.resize(f, (self.in_size, self.in_size), interpolation=cv2.INTER_LINEAR) for f in frames_in_large]
        ...
        if self.color_jitter_prob is not None and (np.random.uniform() < self.color_jitter_prob):
            ...
            frames_in_large = [self.color_jitter(f, self.color_jitter_shift) for f in frames_in_large]
        # random to gray (only for lq)
        if self.gray_prob and np.random.uniform() < self.gray_prob:
            # img_in = cv2.cvtColor(img_in, cv2.COLOR_BGR2GRAY)
            # img_in = np.tile(img_in[:, :, None], [1, 1, 3])
        ...
        frames_in_large = img2tensor(frames_in_large, bgr2rgb=True, float32=True) # list
        ...
        if self.color_jitter_pt_prob is not None and (np.random.uniform() < self.color_jitter_pt_prob):
            ...
            frames_in_large = [self.color_jitter_pt(f, brightness, contrast, saturation, hue) for f in frames_in_large]


        frames_in = torch.stack(frames_in, dim=0)
        frames_in_large = torch.stack(frames_in_large, dim=0)
        frames_gt = torch.stack(frames_gt, dim=0)
        # round and clip
        # img_in = np.clip((img_in * 255.0).round(), 0, 255) / 255.
        # img_in_large = np.clip((img_in_large * 255.0).round(), 0, 255) / 255.
        frames_in = np.clip((frames_in * 255.0).round(), 0, 255) / 255.
        frames_in_large = np.clip((frames_in_large * 255.0).round(), 0, 255) / 255.
        frames_gt = np.clip((frames_gt * 255.0).round(), 0, 255) / 255.

        # Set vgg range_norm=True if use the normalization here
        # normalize
        normalize(frames_in, self.mean, self.std, inplace=True)
        normalize(frames_in_large, self.mean, self.std, inplace=True)
        normalize(frames_gt, self.mean, self.std, inplace=True)
        
        frames_in = frames_in.contiguous()
        frames_in_large = frames_in_large.contiguous()
        frames_gt = frames_gt.contiguous()

        return_dict = {'lq': frames_in, 'lq_large_de': frames_in_large, 'gt': frames_gt, 'gt_path': basename,'lq_path': basename}
